...
def test_exchange_calendar_new_api_names() -> None:
    ...
    assert sessions[2][1] == datetime(2024, 7, 3, 17, 0, tzinfo=UTC)
...
def test_nyse_extended_hours_are_exposed() -> None:
    ...
    assert windows == [
        (
            "pre_open",
            datetime(2024, 1, 8, 9, 0, tzinfo=UTC),
...
def test_nyse_after_close_starts_at_early_close() -> None:
    ...
    assert windows[1] == (
        ...
        datetime(2024, 7, 4, 0, 0, tzinfo=UTC),
    )
...
def test_is_open_at_market_open() -> None:
    ...
    inst = datetime(2024, 1, 8, 14, 30, tzinfo=UTC)
    assert cal.is_open(inst)
    inst_b = datetime(2024, 1, 8, 14, 27, tzinfo=UTC)
...
def test_is_open_handles_dst() -> None:
    ...
    inst = datetime(2024, 3, 11, 13, 30, tzinfo=UTC)
    assert cal.is_open(inst)


def test_is_open_closed_on_weekend() -> None:
    cal = Calendar.from_exchange("XNYS")
    inst = datetime(2024, 1, 6, 15, 0, tzinfo=UTC)  # Sat
...
def test_cme_futures_open_sunday_evening_chicago() -> None:
    ...
    inst = datetime(2024, 1, 7, 23, 0, tzinfo=UTC)
    assert cal.is_open(inst)


def test_nymex_energy_daily_maintenance_break_is_closed() -> None:
    cal = Calendar.from_exchange("XNYM")
    maintenance_break = datetime(2024, 1, 8, 22, 30, tzinfo=UTC)  # 16:30 CT

    assert cal.is_open(maintenance_break) is False
    assert cal.next_open(maintenance_break) == datetime(2024, 1, 8, 23, 0, tzinfo=UTC)
...
def test_cbot_grain_futures_expose_split_sessions() -> None:
    ...
    morning_break = datetime(2024, 1, 8, 14, 0, tzinfo=UTC)  # 08:00 CT
    assert cal.is_open(morning_break) is False
    assert cal.next_open(morning_break) == datetime(2024, 1, 8, 14, 30, tzinfo=UTC)
    assert cal.next_close(morning_break) == datetime(2024, 1, 8, 19, 20, tzinfo=UTC)
...
def test_forex_open_continuously_during_week() -> None:
    ...
    inst = datetime(2024, 1, 9, 8, 0, tzinfo=UTC)
    assert cal.is_open(inst)


def test_crypto_open_on_saturday() -> None:
    cal = Calendar.from_exchange("CRYPTO")
    inst = datetime(2024, 1, 13, 3, 0, tzinfo=UTC)
    assert cal.is_open(inst)
...
def test_apac_lunch_break_sessions_are_exposed() -> None:
    cases = {
        "XTKS": (
            ...
            [
                (datetime(2026, 5, 26, 0, 0, tzinfo=UTC), datetime(2026, 5, 26, 2, 30, tzinfo=UTC)),
                (datetime(2026, 5, 26, 3, 30, tzinfo=UTC), datetime(2026, 5, 26, 6, 30, tzinfo=UTC)),
            ],
        ),
        "XHKG": (
            [(9, 30, 0, 12, 0, 0), (13, 0, 0, 16, 0, 0)],
            [
                (datetime(2026, 5, 26, 1, 30, tzinfo=UTC), datetime(2026, 5, 26, 4, 0, tzinfo=UTC)),
                (datetime(2026, 5, 26, 5, 0, tzinfo=UTC), datetime(2026, 5, 26, 8, 0, tzinfo=UTC)),
            ],
        ),
        "XSHG": (
            [(9, 30, 0, 11, 30, 0), (13, 0, 0, 15, 0, 0)],
            [
                (datetime(2026, 5, 26, 1, 30, tzinfo=UTC), datetime(2026, 5, 26, 3, 30, tzinfo=UTC)),
                (datetime(2026, 5, 26, 5, 0, tzinfo=UTC), datetime(2026, 5, 26, 7, 0, tzinfo=UTC)),
            ],
        ),
    }
...
def test_tokyo_lunch_gap_is_closed_and_boundaries_advance() -> None:
    ...
    lunch_gap = datetime(2026, 5, 25, 2, 45, tzinfo=UTC)

    assert cal.is_open(lunch_gap) is False
    assert cal.next_open(lunch_gap) == datetime(2026, 5, 25, 3, 30, tzinfo=UTC)
    assert cal.next_close(lunch_gap) == datetime(2026, 5, 25, 6, 30, tzinfo=UTC)
...
def test_tokyo_uses_historical_close_before_2024_schedule_change() -> None:
    ...
    assert before[1][1] == datetime(2024, 11, 1, 6, 0, tzinfo=UTC)
    assert after[1][1] == datetime(2024, 11, 5, 6, 30, tzinfo=UTC)
    assert cal.is_open(datetime(2024, 11, 1, 6, 15, tzinfo=UTC)) is False
    assert cal.is_open(datetime(2024, 11, 5, 6, 15, tzinfo=UTC)) is True


def test_next_boundaries_are_inclusive_at_exact_session_times() -> None:
    cal = Calendar.from_exchange("XTKS")
    exact_afternoon_open = datetime(2026, 5, 25, 3, 30, tzinfo=UTC)
    exact_morning_close = datetime(2026, 5, 25, 2, 30, tzinfo=UTC)
...
def test_nyse_july3_closed_after_early_close() -> None:
    ...
    aware = datetime(2024, 7, 3, 18, 0, tzinfo=UTC)  # 14:00 ET (EDT = UTC-4)
...
def test_sessions_include_early_close() -> None:
    ...
    jul3_close = sess[2][1]
    assert jul3_close == datetime(2024, 7, 3, 17, 0, tzinfo=UTC)
    # Jul 5 close is regular 16:00 ET = 20:00 UTC.
    jul5_close = sess[3][1]
    assert jul5_close == datetime(2024, 7, 5, 20, 0, tzinfo=UTC)
